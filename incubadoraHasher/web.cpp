...
void handleRoot() {
  ...
  html += "});}";
  html += "function updateHistory(){fetch('/api/history').then(r=>r.json()).then(d=>{";
  html += "const labels=new Array(d.count).fill('');";
  html += "if(chart) chart.destroy();";
  html += "chart=new Chart(document.getElementById('historyChart'),{type:'line',data:{labels:labels,datasets:[";
  html += "{label:'Temp',data:d.t,borderColor:'red',backgroundColor:'rgba(255,0,0,0.2)'},";
  html += "{label:'Hum',data:d.h,borderColor:'cyan',backgroundColor:'rgba(0,255,255,0.2)'}";
  html += "]}});";
  html += "});}";
...
void handleApiHistory() {
  ...
  for (unsigned int i = 0; i < count; i++) {
    unsigned int idx = historyFull ? (historyIndex + i) % HISTORY_SIZE : i;
